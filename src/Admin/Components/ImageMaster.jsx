...
const ImageMaster = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const response = await httpClient.get('/get__all__image__master');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        ...
        const response = await httpClient.put(`/update__image__master/${editingItem.id}`, {
        ...
        // Add new item
        const response = await httpClient.post('/create__image__master', {
          imageTitle: formData.imageTitle,
          image: formData.image
        });
  ...
  const handleConfirmDelete = async () => {
    if (itemToDelete) {
      setLoading(true);
      try {
        const response = await httpClient.delete(`/delete__image__master/${itemToDelete}`);
  ...
  return (
    <div className="image-master-container">
      <ToastContainer 
        position="top-right"
        autoClose={3000}
      ...
      <div className="header">
        <div>
          <h2 className="title">Image Master</h2>
          <p className="subtitle">Manage image gallery</p>
        </div>
        <button 
          className="add-btn"
        ...
        <div className="modal-overlay" onClick={resetForm}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editingItem ? 'Edit Image' : 'Add New Image'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-actions">
                ...
                <button type="submit" className="submit-btn" disabled={loading || uploading}>
                  {loading ? 'Processing...' : (editingItem ? 'Update Image' : 'Add Image')}
                </button>
              </div>
            </form>
          </div>
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="no-data">
          <p>No images found. Click "Add New Image" to create one.</p>
        </div>
      ) : (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="title-cell">{item.imageTitle}</td>
                  <td>
